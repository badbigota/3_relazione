...
int main(){
	...
	for(int i=0;i<9;i++){
		fout<<"Estensimetro "<<est[i].n_est<<endl<<est[i].lunghezza<<endl<<est[i].diametro<<endl;
		for(int j=0;j<10;j++)	fout<<est[i].allungamento[j]<<endl<<est[i].accorciamento[j]<<endl;
		fout<<endl;
	}
	return 0;
}
